...
type FileServiceClient interface {
	...
	UploadAndNotify(ctx context.Context, opts ...grpc.CallOption) (FileService_UploadAndNotifyClient, error)
}
...
func (c *fileServiceClient) UploadAndNotify(ctx context.Context, opts ...grpc.CallOption) (FileService_UploadAndNotifyClient, error) {
	stream, err := c.cc.NewStream(ctx, &FileService_ServiceDesc.Streams[2], "/file.FileService/UploadAndNotify", opts...)
	if err != nil {
		return nil, err
	}
	x := &fileServiceUploadAndNotifyClient{stream}
	return x, nil
}

type FileService_UploadAndNotifyClient interface {
	Send(*UploadAndNotifyProgressRequest) error
	Recv() (*UploadAndNotifyProgressResponse, error)
	grpc.ClientStream
}

type fileServiceUploadAndNotifyClient struct {
	grpc.ClientStream
}

func (x *fileServiceUploadAndNotifyClient) Send(m *UploadAndNotifyProgressRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *fileServiceUploadAndNotifyClient) Recv() (*UploadAndNotifyProgressResponse, error) {
...
type FileServiceServer interface {
	...
	UploadAndNotify(FileService_UploadAndNotifyServer) error
...
func (UnimplementedFileServiceServer) UploadAndNotify(FileService_UploadAndNotifyServer) error {
...
func _FileService_UploadAndNotify_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(FileServiceServer).UploadAndNotify(&fileServiceUploadAndNotifyServer{stream})
}

type FileService_UploadAndNotifyServer interface {
	Send(*UploadAndNotifyProgressResponse) error
	Recv() (*UploadAndNotifyProgressRequest, error)
	grpc.ServerStream
}

type fileServiceUploadAndNotifyServer struct {
	grpc.ServerStream
}

func (x *fileServiceUploadAndNotifyServer) Send(m *UploadAndNotifyProgressResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *fileServiceUploadAndNotifyServer) Recv() (*UploadAndNotifyProgressRequest, error) {
...
var FileService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		},
		{
			...
			Handler:       _FileService_UploadAndNotify_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
